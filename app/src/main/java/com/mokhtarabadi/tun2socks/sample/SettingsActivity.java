...
public class SettingsActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding.excludedApps.setOnClickListener(v -> startActivity(new Intent(SettingsActivity.this, ExcludedAppsActivity.class)));
    ...
    private void saveFromUI() {
        preferences.edit().putBoolean(PREFERENCE_SUPPORT_IPV4, binding.supportIpv4.isChecked())
                .putBoolean(PREFERENCE_SUPPORT_IPV6, binding.supportIpv6.isChecked())
                .putStringSet(PREFERENCE_IPV4_DNS_SERVERS, new HashSet<>(Arrays.asList(binding.ipv4DnsServers.getText().toString().trim().split(","))))
                .putStringSet(PREFERENCE_IPV6_DNS_SERVERS, new HashSet<>(Arrays.asList(binding.ipv6DnsServers.getText().toString().trim().split(","))))
